...
[Conversion-Rules.md](docs/Conversion-Rules.md), 현재 공식 API 감사는
[Current-API-Audit.md](docs/Current-API-Audit.md)를 따른다.
...
| 공개 메서드 | 계약 |
|---|---|
| `build_request(messages, tools=(), model=None, **params)` | 대상 API의 wire body를 반환. 전송 없음 |
...
| `CitationBlock` | 답변 좌표와 원문 좌표를 분리한 인용 |
...
| custom Vocabulary/Citations | 완료 | lift/lower 역변환과 요청 재전송 |
...
| 멀티모달 | base64, URL/URI, file ID, MIME별 Image/Audio/Document |
