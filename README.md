...
# 명시한 None은 그 요청에서 필드를 지운다. 쓰지 않은 것과 구분된다.
body = bridge.build_request(
    ["질문"],
    hyperparameters=Hyperparameters(temperature=None),
)
```

`None`을 명시하면 생성자 기본값이 그 요청에서만 해제된다. 쓰지 않은 필드는 기본값이 그대로
유지되므로, 한 필드만 끄려고 나머지를 다시 적을 필요가 없다.
...
유효한지는 판정하지 않는다. 같은 API라도 모델에 따라 받는 필드와 값 범위가 다르고, 어떤 필드는
beta 헤더를 함께 요구한다. 그 목록은 코드에도 이 문서에도 담지 않는다 — 모델이 나올 때마다
낡고, 낡은 채로 남으면 조용히 틀린 정보가 된다.

**각 벤더의 사용 전략은 해당 API 문서를 참고한다.** 어긋나면 벤더가 400으로 알려주고, 그 응답
본문은 `TransportError.detail`에 실린다.
